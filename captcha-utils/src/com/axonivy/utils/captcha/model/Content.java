...
public class Content {
  ...
  public ContentProducer getContentProducer() {
    int length = this.length != null ? this.length : 6;
    return switch (type) {
      case LATIN -> new LatinContentProducer(length);
      case ARABIC -> new ArabicContentProducer(length);
      case NUMBERS -> new NumbersContentProducer(length);
      case CHINESE -> new ChineseContentProducer(length);
      case STATIC_LIST -> new FiveLetterFirstNameContentProducer();
      default -> new LatinContentProducer();
    };
  }
  ...
  public WordRenderer getDefaultWordRenderer() {
    ...
    List<Color> colors = emptyIfNull(this.colors).stream().map(ColorHelper::hexToColor).filter(Optional::isPresent)
